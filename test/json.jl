...
# This verifies that StructUtils correctly dispatches through @choosetype
# AbstractADType when deserializing a struct field declared as AbstractADType.
...
@testset "AutoForwardDiff JSON structure" begin
    obj = JSON.parse(write_ad(AutoForwardDiff()))
    @test obj["type"] == "AutoForwardDiff"
    @test isnothing(obj["chunksize"])

    obj2 = JSON.parse(write_ad(AutoForwardDiff(; chunksize = 8)))
...
@testset "AutoReverseDiff JSON structure" begin
    obj = JSON.parse(write_ad(AutoReverseDiff()))
    @test obj["type"] == "AutoReverseDiff"
    @test obj["compile"] == false

    obj2 = JSON.parse(write_ad(AutoReverseDiff(; compile = true)))
...
@testset "AutoEnzyme JSON structure" begin
    obj = JSON.parse(write_ad(AutoEnzyme()))
    @test obj["type"] == "AutoEnzyme"
    @test isnothing(obj["mode"])

    obj2 = JSON.parse(write_ad(AutoEnzyme(mode = EnzymeCore.Forward)))
    @test obj2["mode"] == "Forward"

    obj3 = JSON.parse(write_ad(AutoEnzyme(mode = EnzymeCore.Reverse)))
...
@testset "AutoTaylorDiff JSON structure" begin
    obj = JSON.parse(write_ad(AutoTaylorDiff()))
    @test obj["type"] == "AutoTaylorDiff"
    @test obj["order"] == 1

    obj2 = JSON.parse(write_ad(AutoTaylorDiff(; order = 3)))
    @test obj2["order"] == 3
end
...
@testset "AutoHyperHessians JSON structure" begin
    obj = JSON.parse(write_ad(AutoHyperHessians()))
    @test obj["type"] == "AutoHyperHessians"
    @test isnothing(obj["chunksize"])
    @test obj["simd"] == false
    @test obj["jet"] == false

    obj2 = JSON.parse(write_ad(AutoHyperHessians(; chunksize = 4, simd = true, jet = true)))
...
@testset "AutoPolyesterForwardDiff JSON structure" begin
    obj = JSON.parse(write_ad(AutoPolyesterForwardDiff()))
    @test obj["type"] == "AutoPolyesterForwardDiff"
    @test isnothing(obj["chunksize"])

    obj2 = JSON.parse(write_ad(AutoPolyesterForwardDiff(; chunksize = 8)))
    @test obj2["chunksize"] == 8
end
...
@testset "AutoGTPSA JSON structure" begin
    obj = JSON.parse(write_ad(AutoGTPSA()))
...
@testset "AutoMooncake JSON structure" begin
    obj = JSON.parse(write_ad(AutoMooncake()))
...
@testset "AutoMooncakeForward JSON structure" begin
    obj = JSON.parse(write_ad(AutoMooncakeForward()))
...
@testset "AutoTapir JSON structure" begin
    obj = JSON.parse(write_ad(AutoTapir(false)))  # positional: avoids depwarn
    @test obj["type"] == "AutoTapir"
    @test obj["safe_mode"] == false

    obj2 = JSON.parse(write_ad(AutoTapir(true)))
...
@testset "AutoFiniteDiff JSON structure" begin
    obj = JSON.parse(write_ad(AutoFiniteDiff()))
...
@testset "AutoReactant JSON structure" begin
    obj = JSON.parse(write_ad(AutoReactant()))
